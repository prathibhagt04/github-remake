      ...
      function reloadPage() {
        ...
        switch (activePage.innerHTML) {
              ...
              fetch(`https://api.github.com/users/${res.login}/repos`)
                ...
                .then(repores => {
                  repores.forEach(repo => {
                    fetch(
                      `https://api.github.com/repos/${res.login}/${repo.name}/branches`
                    )
                      .then(allBranches => allBranches.json())
                      .then(allBranches => {
                        function checkBranch() {
                          let check = false;
                          if (allBranches != []) {
                            allBranches.forEach(branch => {
                              if (branch.name === 'gh-pages') check = true;
                            });
                          }
                          return check;
                        }
                        ...
                        if (check) {
                          ...
                          repoDiv.innerHTML = `<a class="repo-box" href="https://${res.login}.github.io/${repo.name}" target="_blank"><h3 style="text-decoration: underline;">${repo.name}</h3><h5>${repo.description}</h5></a>`;
          ...
          case 'All Repos':
            {
              ...
              fetch(`https://api.github.com/users/${res.login}/repos`)
                ...
                .then(repores => {
                  ...
                  repores.forEach(repo => {
                    let repoDiv = document.createElement('div');
                    let descr = repo.description
                      ? repo.description
                      : 'No description available';
                    repoDiv.innerHTML = `<a class="repo-box" href="https://github.com/${res.login}/${repo.name}" target="_blank"><h3 style="text-decoration: underline;">${repo.name}</h3><h5>${descr}</h5></a>`;
                    activePageContainer.append(repoDiv);
                  });
                })
          ...
          case 'About Github':
            {
              activePageContainer.innerHTML = '';
              gitDivContainer.classList.add('add-flex');
      ...
      function addingEventListeners() {
        tabElements.forEach(element => {
          element.addEventListener('click', () => {
            document
              .querySelector('.tabElement.active')
